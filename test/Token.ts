...
describe("Token contract", () => {
    ...
    describe("Swap", () => {
        ...
        */
        ...
        it("holde change list", async () => {

            let lotteryHolderMinBalance = (await token.lotteryHolderMinBalance()).toString();
            lotteryHolderMinBalance = lotteryHolderMinBalance * 2;
            // update ticket value for 2b
            await token.setLotteryHolderMinBalance(lotteryHolderMinBalance.toString());
            lotteryHolderMinBalance = (await token.lotteryHolderMinBalance()).toString();
            expect(fromGWei(lotteryHolderMinBalance)).to.be.eq('2000000000.0')
            console.log('lotteryHolderMinBalance', fromGWei(lotteryHolderMinBalance));
            ...
            await router.connect(USER).swapExactETHForTokensSupportingFeeOnTransferTokens
            (0, [weth.address, token.address], user, 9646498066, {from: user, value: toWei('0.002')});

            await router.connect(USER1).swapExactETHForTokensSupportingFeeOnTransferTokens
            (0, [weth.address, token.address], user1, 9646498066, {from: user1, value: toWei('0.002')});

            await router.connect(USER2).swapExactETHForTokensSupportingFeeOnTransferTokens
            (0, [weth.address, token.address], user2, 9646498066, {from: user2, value: toWei('0.002')});

            await router.connect(USER3).swapExactETHForTokensSupportingFeeOnTransferTokens
            (0, [weth.address, token.address], user3, 9646498066, {from: user3, value: toWei('0.002')});

            let getTicketsByBalance = (await token.getTicketsByBalance()).length;
            console.log('getTicketsByBalance', getTicketsByBalance);
            expect(getTicketsByBalance).to.be.eq(0)

            const balanceOfUser = ((await token.balanceOf(user)).toString());
            const balanceOfUser1 = ((await token.balanceOf(user1)).toString());
            const balanceOfUser2 = ((await token.balanceOf(user2)).toString());
            const balanceOfUser3 = ((await token.balanceOf(user3)).toString());

            console.log(fromGWei(balanceOfUser));
            console.log(fromGWei(balanceOfUser1));
            console.log(fromGWei(balanceOfUser2));
            console.log(fromGWei(balanceOfUser3));
            // all users should have between 1.8b to 1.7b
            expect(fromGWei(balanceOfUser)).to.be.eq('1811072314.385984554') // 906m
            expect(fromGWei(balanceOfUser1)).to.be.eq('1803867666.924288942') // 904m
            expect(fromGWei(balanceOfUser2)).to.be.eq('1796705946.812940753') // 902m
            expect(fromGWei(balanceOfUser3)).to.be.eq('1789586813.612176779') // 900m

            lotteryHolderMinBalance = lotteryHolderMinBalance / 2;
            // update ticket value for 1b
            await token.setLotteryHolderMinBalance(lotteryHolderMinBalance.toString());
            lotteryHolderMinBalance = (await token.lotteryHolderMinBalance()).toString();
            expect(fromGWei(lotteryHolderMinBalance)).to.be.eq('1000000000.0')
            console.log('lotteryHolderMinBalance', fromGWei(lotteryHolderMinBalance));


            const listOfUsers = [user, user1, user2, user3];
            await token.updateHolderList(listOfUsers);

            getTicketsByBalance = (await token.getTicketsByBalance()).length;
            console.log('getTicketsByBalance', getTicketsByBalance);
            expect(getTicketsByBalance).to.be.eq(4)
        ...
        it("test anti-abuse with 5 bnb", async () => {
            ...
            await router.connect(USER).swapExactETHForTokensSupportingFeeOnTransferTokens
            (0, [weth.address, token.address], user, 9646498066, {from: user, value: toWei('0.0006')});

            balanceOf = (await token.balanceOf(user)).toString();
            // console.log('balance of user', fromGWei(balanceOf));
            expect( fromGWei(balanceOf) ).to.be.eq('544039809.831900975')
